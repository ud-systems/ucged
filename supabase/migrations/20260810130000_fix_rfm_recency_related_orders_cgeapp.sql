-- Fix Days quiet / RFM recency for duplicate Shopify profiles.
-- 9999 meant "no orders on this customer_id" even when a related account
-- (same email local-part / phone) had recent orders.
...
BEGIN
  ...
  agg AS (
    SELECT
      tc.id AS customer_id,
      CASE
        WHEN max(ro.order_ts) IS NULL THEN NULL
  ...
  scored_raw AS (
    SELECT
      ...
      CASE
        ...
        ELSE 6 - ntile(5) OVER (ORDER BY coalesce(a.recency_days, 9999) ASC, a.customer_id)
      END AS r_score,
      CASE
        WHEN a.frequency_orders = 0 THEN 1
  ...
  scored AS (
    SELECT
      ...
      CASE
        WHEN sr.frequency_orders = 0 THEN 'Lost'
...
-- Clear bogus forever-ago sentinel; recompute will fill correct values
UPDATE public.shopify_customers
SET rfm_recency_days = NULL
WHERE rfm_recency_days >= 9999;
