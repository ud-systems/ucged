-- Add supervisor role and treat supervisors as team viewers on performance RPCs.

ALTER TYPE public.app_role ADD VALUE IF NOT EXISTS 'supervisor';

CREATE OR REPLACE FUNCTION public.is_cge_team_viewer(_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.is_cge_team_viewer(UUID) TO authenticated, service_role;
-- ---------------------------------------------------------------------------
-- Summary
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  v_is_admin := public.is_cge_team_viewer(_viewer_user_id);
