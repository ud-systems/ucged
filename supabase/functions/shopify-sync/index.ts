...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // PostgREST defaults to 1000 rows — must page or most orders stay unlinked.
    const customerIdByShopify = new Map<string, string>();
    {
      const pageSize = 1000;
      let from = 0;
      while (true) {
        const { data: customerRows, error: custMapErr } = await supabase
          .from("shopify_customers")
          .select("id, shopify_customer_id")
          .range(from, from + pageSize - 1);
        if (custMapErr) throw custMapErr;
        const batch = customerRows || [];
        for (const r of batch as { id: string; shopify_customer_id: string }[]) {
          if (r.shopify_customer_id) customerIdByShopify.set(r.shopify_customer_id, r.id);
        }
